...
export function ProductList({ products, onEdit, onDelete, onToggleActive }: ProductListProps) {
  ...
  const handleEdit = (product: Product) => {
    try {
      ...
      showToast(err?.message || "Erro ao editar produto.");
    }
  };
  const handleDelete = (id: string) => {
    try {
      ...
      showToast(err?.message || "Erro ao excluir produto.");
    }
  };
  const handleToggleActive = (product: Product, active: boolean) => {
    try {
      onToggleActive && onToggleActive(product, active);
    } catch (err: any) {
      showToast(err?.message || "Erro ao alterar status do produto.");
    }
  };
                  ...
                  <tr key={product.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                    ...
                    <td className="px-4 sm:px-6 py-4">
                      <Switch
                        ...
                        onChange={(value) => handleToggleActive(product, value)}
